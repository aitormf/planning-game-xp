...
describe('user.js — resolveUserConfigPath', () => {
  ...
  it('should resolve to project root when MCP_INSTANCE_DIR is not set', async () => {
    delete process.env.MCP_INSTANCE_DIR;

    // Dynamic import to re-evaluate with current env
    const { resolveUserConfigPath } = await import('../user.js?t=no-instance-' + Date.now());
    const result = resolveUserConfigPath();

    // user.js resolves to __dirname/.. which is the project root (not mcp/)
    expect(result).toBe(resolve(projectRoot, 'mcp.user.json'));
  });

  it('should resolve to instance dir when MCP_INSTANCE_DIR is set', async () => {
    process.env.MCP_INSTANCE_DIR = '/tmp/test-instances/pro';

    const { resolveUserConfigPath } = await import('../user.js?t=with-instance-' + Date.now());
    const result = resolveUserConfigPath();
  ...
  it('should handle MCP_INSTANCE_DIR with trailing slash', async () => {
    ...
    const { resolveUserConfigPath } = await import('../user.js?t=trailing-slash-' + Date.now());
    const result = resolveUserConfigPath();
